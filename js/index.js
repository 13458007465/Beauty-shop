...
var aIdots = carousel.querySelectorAll('.dotItem');
/*******************全局变量定义***********************/
/*
 * 1.默认选中图片下标
 * 2.动画执行状态
...
//设置定时器（每隔一定时间执行一次）
    var tim = setInterval(function () {
        ...
        if ((offset < 0 && curLeft > tabLeft) || (offset > 0 && curLeft < tabLeft)) {
            ...
            if (curLeft < -1025) {
                imgbox.style.left = "-205px";
            } else if (curLeft > -205) {
                imgbox.style.left = "-1025px";
            }
        }
    }, interval)
...
//	循环绑定事件
    aIdots[i].onclick = function () {
        ...
        var offset = -205 * (this.index - curImgIdx);
        //	调用切换图片函数
        tab(offset);
        //	更新当前下标
